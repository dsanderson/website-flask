...
def fetch_document_data(doc_ids, query):
    ...
    for q in query:
        if q[1]!='':
            ...
            db_query = session.query(Scraped_Site.id, sqla.func.array_agg(Unit_Num.value)).join(Unit_Num).filter(Scraped_Site.id==Unit_Num.source).filter(Unit_Num.unit_type==q[1]).filter(Scraped_Site.id.in_(ids)).group_by(Scraped_Site.id)
